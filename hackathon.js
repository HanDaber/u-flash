 ...
 */
  ...
  app.use(function(req, res, next) {
    if (req.path === '/api/upload' || req.path === '/sms' || req.path === '/sms/') {
      next();
    } else {
  ...
  app.use(function(req, res, next) {
    res.locals.backend_uri = process.env.BE_URI;
    next();
  });
 ...
 */
  ...
  // app.get('/trackers', passportConfig.isAuthenticated, homeController.index );
  // app.get('/shipments', passportConfig.isAuthenticated, homeController.index );
  app.get('/:resource', passportConfig.isAuthenticated, homeController.index );
  app.post('/sms', smsController.postSms );
...
app.listen(app.get('port'), function(){
  console.log('Frontend server listening on port %d in %s mode', app.get('port'), app.get('env') );
});
...
backend.on('listening', function() {
  console.log('Backend server listening on port %d in %s mode', 4000, app.get('env') );
});
